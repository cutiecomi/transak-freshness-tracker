...
const FRESHNESS_CONFIG = {
  fresh: { label: "Fresh", emoji: "🟢", bg: "bg-emerald-500/10", text: "text-emerald-400", border: "border-emerald-500/20" },
  aging: { label: "Aging", emoji: "🟡", bg: "bg-yellow-500/10", text: "text-yellow-400", border: "border-yellow-500/20" },
  stale: { label: "Stale", emoji: "🟠", bg: "bg-orange-500/10", text: "text-orange-400", border: "border-orange-500/20" },
  "needs-update": { label: "Needs Update", emoji: "🔴", bg: "bg-red-500/10", text: "text-red-400", border: "border-red-500/20" },
};

const CONTENT_TYPE_CONFIG = {
  evergreen: { label: "Evergreen", icon: "🌿", color: "text-emerald-400" },
  "semi-evergreen": { label: "Semi-evergreen", icon: "🌤️", color: "text-yellow-400" },
  "time-sensitive": { label: "Time-sensitive", icon: "⏰", color: "text-orange-400" },
...
function StatCard({ label, value, sub, color, active, onClick }: { label: string; value: number; sub?: string; color: string; active?: boolean; onClick?: () => void }) {
  return (
    <div onClick={onClick}
      className={`rounded-xl border ${color} bg-[#151c2c] p-5 flex flex-col gap-1 cursor-pointer transition-all hover:bg-[#1a2338] hover:scale-[1.02] ${active ? "ring-2 ring-[#0364FF] ring-offset-2 ring-offset-[#0c111d]" : ""}`}>
    ...
    <div className="max-w-[1600px] mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Article Freshness Tracker</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        <StatCard label="🌿 Evergreen" value={contentCounts.evergreen} sub="Concepts & explainers" color="border-emerald-500/20"
          active={filterContentType === "evergreen"} onClick={() => { setFilterContentType(filterContentType === "evergreen" ? "" : "evergreen"); setFilterFreshness(""); }} />
        <StatCard label="🌤️ Semi-evergreen" value={contentCounts["semi-evergreen"]} sub="Guides & tutorials" color="border-yellow-500/20"
        ...
        <StatCard label="⏰ Time-sensitive" value={contentCounts["time-sensitive"]} sub="Year-specific content" color="border-orange-500/20"
        ...
        <StatCard label="📰 News" value={contentCounts.news} sub="Partnerships & launches" color="border-blue-500/20"
          active={filterContentType === "news"} onClick={() => { setFilterContentType(filterContentType === "news" ? "" : "news"); setFilterFreshness(""); }} />
      </div>

      {/* Legend */}
      <div className="bg-[#151c2c] rounded-xl border border-[#1e293b] p-4 mb-6">
        <div className="text-xs text-gray-500 mb-2 font-medium uppercase tracking-wide">How freshness is determined</div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-gray-300">
          <div><span className="text-emerald-400 font-semibold">🌿 Evergreen</span> — "What is X", explainers, definitions → stays fresh up to 2 years</div>
          <div><span className="text-yellow-400 font-semibold">🌤️ Semi-evergreen</span> — How-to guides, tutorials → 12 month shelf life (UIs change)</div>
          <div><span className="text-orange-400 font-semibold">⏰ Time-sensitive</span> — Year references, events, predictions → expires with the year</div>
          <div><span className="text-blue-400 font-semibold">📰 News</span> — Partnerships, launches → 2 year check (verify partner still active)</div>
        </div>
      </div>
      ...
      <div className="bg-[#151c2c] rounded-xl border border-[#1e293b] p-4 mb-6">
        <div className="flex flex-wrap gap-3">
          <input
            ...
            className="bg-[#0c111d] border border-[#1e293b] rounded-lg px-4 py-2 text-sm text-gray-200 placeholder-gray-500 flex-1 min-w-[200px] focus:outline-none focus:border-[#0364FF] focus:ring-1 focus:ring-[#0364FF]/30 transition-colors"
          />
          <select value={filterContentType} onChange={(e) => setFilterContentType(e.target.value)}
            className="bg-[#0c111d] border border-[#1e293b] rounded-lg px-3 py-2 text-sm text-gray-300 focus:outline-none focus:border-[#0364FF]">
          ...
          {activeFilters > 0 && (
            <button onClick={() => { setSearch(""); setFilterCategory(""); setFilterFreshness(""); setFilterTag(""); setFilterContentType(""); }}
              className="px-4 py-2 rounded-lg text-sm border border-[#1e293b] text-gray-400 hover:text-white hover:border-gray-500 transition-colors">
              Clear all
            </button>
          )}
        </div>
      ...
      {/* Table */}
      <div className="bg-[#151c2c] rounded-xl border border-[#1e293b] overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#1e293b] bg-[#0c111d] text-gray-400">
                {([
                  ["title", "Title"],
                  ["contentType", "Type"],
                  ["categories", "Category"],
                  ...
                  <th key={key} onClick={() => toggleSort(key)}
                    className="text-left px-4 py-3 font-medium cursor-pointer hover:text-white transition-colors select-none whitespace-nowrap text-xs uppercase tracking-wide">
                    {label} <span className="text-gray-600">{sortIcon(key)}</span>
                  </th>
                ))}
                <th className="text-left px-4 py-3 font-medium text-xs uppercase tracking-wide text-gray-400 whitespace-nowrap">Reasoning</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((article) => {
                ...
                return (
                  <tr key={article.id} className="border-b border-[#1e293b]/50 hover:bg-[#1a2338] transition-colors">
                    <td className="px-4 py-3 max-w-sm">
                      <a href={article.url} target="_blank" rel="noopener noreferrer"
                        className="text-[#4d94ff] hover:text-[#79b0ff] hover:underline transition-colors line-clamp-2 font-medium">
                        {article.title}
                      </a>
                      {article.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1">
                          {article.tags.slice(0, 3).map((t) => (
                            <span key={t} className="text-[10px] px-1.5 py-0.5 rounded bg-[#0c111d] text-gray-500 border border-[#1e293b]">{t}</span>
                          ))}
                    ...
                    <td className="px-4 py-3 min-w-[250px]">
                      <span className="text-xs text-gray-400 leading-relaxed">{article.reasoning}</span>
                    </td>
                  </tr>
                );
              })}
